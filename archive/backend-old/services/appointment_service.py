...
def create_appointment(
    ...
    try:
        ...
        item = {
            ...
            'created_at': current_time,        # Timestamp attribute
...
# =============================================================================
# APPOINTMENT RETRIEVAL (uses query by partition key)
# =============================================================================

def get_appointment_by_id(appointment_id: str) -> Optional[Dict]:
    """Get appointment details by ID using query().

    Current table schema:
        Partition Key: appointment_id (String)
        Sort Key: session_id (String)

    Since we only have appointment_id, we use query() instead of get_item().
    ...
    Example:
        >>> appt = get_appointment_by_id("abc-123")
        >>> if appt:
    ...
    try:
        ...
        def _query_appointment():
            # Query by partition key only (since we don't have session_id)
            return appointments_table.query(
                KeyConditionExpression=Key('appointment_id').eq(appointment_id),
                Limit=1
            )

        response = retry_with_backoff(_query_appointment)
...
def verify_appointment(appointment_id: str, verification_code: str) -> bool:
    ...
    Example:
        >>> if verify_appointment("abc-123", "123456"):
    ...
    try:
        ...
        def _update_appointment():
            ...
            # Need both partition key (appointment_id) and sort key (session_id)
...
def get_appointment_status(appointment_id: str) -> Optional[str]:
    ...
    Example:
        >>> status = get_appointment_status("abc-123")
        >>> # Returns: "verified"
    """
    appointment = get_appointment_by_id(appointment_id)
